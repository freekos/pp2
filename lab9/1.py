import random
import time

import pygame as py
import sys
from pygame.locals import *

import utils

py.init()

FPS = 60
FramePerSec = py.time.Clock()

BLUE = (0, 0, 255)
RED = (255, 0, 0)
GREEN = (0, 255, 0)
BLACK = (0, 0, 0)
WHITE = (255, 255, 255)

SCREEN_WIDTH = 400
SCREEN_HEIGHT = 600
SPEED = 5
SCORE = 0
COIN_SCORE = 0
TOTAL_SCORE = 0

font = py.font.SysFont("Verdana", 60)
font_small = py.font.SysFont("Verdana", 20)
game_over = font.render("Game Over", True, BLACK)

background = utils.load_image("Road.png")

DISPLAYSURF = py.display.set_mode((400, 600))
py.display.set_caption("Game")


class Enemy(py.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image = utils.load_image("Enemy.png")
        self.rect = self.image.get_rect()
        self.rect.center = (random.randint(40, SCREEN_WIDTH - 40), 0)

    def move(self):
        global SCORE
        self.rect.move_ip(0, SPEED)
        if self.rect.top > 600:
            SCORE += 1
            self.rect.top = 0
            self.rect.center = (random.randint(40, SCREEN_WIDTH - 40), 0)


class Player(py.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image = utils.load_image("Player.png")
        self.rect = self.image.get_rect()
        self.rect.center = (160, 520)

    def move(self):
        pressed_keys = py.key.get_pressed()

        if self.rect.left > 0:
            if pressed_keys[K_LEFT]:
                self.rect.move_ip(-5, 0)
        if self.rect.right < SCREEN_WIDTH:
            if pressed_keys[K_RIGHT]:
                self.rect.move_ip(5, 0)

    def collect_coin(self, coins):
        collisions = py.sprite.spritecollide(self, coins, True)
        for coin in collisions:
            return True
        return False


class Coin(py.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image = utils.load_image("Coin.svg")
        self.rect = self.image.get_rect()
        self.rect.center = (random.randint(40, SCREEN_WIDTH - 40), 0)

    def move(self):
        self.rect.move_ip(0, SPEED)
        if self.rect.top > 600:
            self.rect.top = 0
            self.rect.center = (random.randint(40, SCREEN_WIDTH - 40), 0)


P1 = Player()
E1 = Enemy()
C1 = Coin()

enemies = py.sprite.Group()
enemies.add(E1)
coins = py.sprite.Group()
coins.add(C1)
all_sprites = py.sprite.Group()
all_sprites.add(P1)
all_sprites.add(E1)
all_sprites.add(C1)

INC_SPEED = py.USEREVENT + 1
py.time.set_timer(INC_SPEED, 1000)

while True:

    for event in py.event.get():
        if event.type == INC_SPEED:
            SPEED += 0.5
        if event.type == QUIT:
            py.quit()
            sys.exit()

    DISPLAYSURF.blit(background, (0, 0))
    scores = font_small.render(str(SCORE), True, BLACK)
    coin_scores = font_small.render(str(COIN_SCORE), True, BLACK)
    total_scores = font_small.render(str(TOTAL_SCORE), True, BLACK)
    DISPLAYSURF.blit(scores, (10, 10))
    DISPLAYSURF.blit(coin_scores, (360, 10))
    DISPLAYSURF.blit(total_scores, (360, 30))

    for entity in all_sprites:
        DISPLAYSURF.blit(entity.image, entity.rect)
        entity.move()

    total_score = random.randint(0, 10)

    if P1.collect_coin(coins):
        if TOTAL_SCORE == 100:
            SPEED += 1.0
        COIN_SCORE += 1
        TOTAL_SCORE += total_score
        new_coin = Coin()
        coins.add(new_coin)
        all_sprites.add(new_coin)

    if py.sprite.spritecollideany(P1, enemies):
        time.sleep(0.5)

        DISPLAYSURF.fill(RED)
        DISPLAYSURF.blit(game_over, (30, 250))

        py.display.update()
        for entity in all_sprites:
            entity.kill()
        time.sleep(2)
        py.quit()
        sys.exit()

    py.display.update()
    FramePerSec.tick(FPS)